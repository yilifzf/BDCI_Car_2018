...
def merge_oof(dir):
    ...
    with open(os.path.join(dir, "1", "test.tsv"), "r", encoding='utf-8') as f:
        reader = f.readlines()
        n_test = len(reader) -1
    print(train_length)
    print(dev_length)
    ...
    for i, (train_index, dev_index) in enumerate(kfold_split(n_train, NFOLDS)):
        fold = i + 1
        dev_index2 = np.loadtxt(os.path.join(dir, str(fold), 'dev.ind'), dtype=int)
        print(dev_index2)
        print(dev_index)
