...
from modeling import BertConfig, BertForSequenceClassification, BertForMultiLabelClassification
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tensor = [0 for i in range(len(label_list))]
...
def accuracy(out, labels):
    out = torch.nn.functional.sigmoid(torch.Tensor(out)).numpy()
    outputs = np.asarray([categories_from_output(output, 0.45) for output in out])
    acc = (outputs == labels).all(axis=1).sum()
    # print(acc)
    # precision, recall, f1, acc2 = score_list(outputs, labels)
    # print(precision, recall, f1, acc2)
    return acc

def accuracy1(out, labels):
    outputs = np.asarray([categories_from_output(output, 0.45) for output in out])
    acc = (outputs == labels).all(axis=1).mean()
    # print(acc)
    precision, recall, f1, acc2 = score_list(outputs, labels)
    # print(precision, recall, f1, acc2)
    return acc
...
def accuracy2(out, labels):
    ...
    outputs = np.asarray([categories_from_output(output, 0.5) for output in out])
    acc = (outputs == labels).all(axis=1).mean()
    print("0.5:")
    print(acc)
    precision, recall, f1, acc2 = score_list(outputs, labels)
...
def accuracy3(out, labels):
    out = np.asarray(out)
    labels = np.asarray(labels)
    outputs = np.asarray([categories_from_output(output, 0.3) for output in out])
    acc = (outputs == labels).all(axis=1).mean()
    print("0.3:")
    print(acc)
    precision, recall, f1, acc2 = score_list(outputs, labels)
...
def accuracy3_2(out, labels):
    out = np.asarray(out)
    labels = np.asarray(labels)
    outputs = np.asarray([categories_from_output(output, 0.2) for output in out])
    acc = (outputs == labels).all(axis=1).mean()
    print("0.2:")
    print(acc)
    precision, recall, f1, acc2 = score_list(outputs, labels)
...
def accuracy7(out, labels):
    out = np.asarray(out)
    labels = np.asarray(labels)
    out = torch.nn.functional.sigmoid(torch.Tensor(out)).numpy()
    outputs = np.asarray([categories_from_output(output, 0.6) for output in out])
    acc = (outputs == labels).all(axis=1).mean()
    print("Sigmoid: 0.6")
    print(acc)
    precision, recall, f1, acc2 = score_list(outputs, labels)
...
def main():
    ...
    for i in range(0,5):
        print("FOLDS:", i)
        args.data_dir = os.path.join(dir, str(i+1))
        train(args)
            ...
            if args.do_eval:
                ...
                if eval_loss < min_loss:
                    ...
                    for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
                        ...
                        np.save(os.path.join(args.data_dir, 'oof_test'), np.asarray(logit_test))
